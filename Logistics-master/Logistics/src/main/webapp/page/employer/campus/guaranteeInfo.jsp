...
<table class="csstest" cellspacing="0" cellpadding="0" id="ContentPlaceHolder1_dg1" style="border-width:0px;border-style:None;width:100%;border-collapse:collapse;">
	<tbody><tr align="left" style="color:#999999;background-color:White;height:20px;">
	 ...
	 <c:forEach items="${repairss}" var="repairs" varStatus="status">
		...
		<td style="width:45%;">
        <a href="${ctx}/repairs/selectById/${repairs.repairsId}" class="csstest">
      ...
      <td style="width:10%;">
       ...
       <td class="userOpreate" style="width:12%;color:#6699cc;">
       	<c:if test="${repairs.repairsStatus == '待维护'}">
       		<a href="${ctx}/repairs/updateById/?repairsId=${repairs.repairsId}&category=${repairs.repairsCategory}&repairsStatus=维护中&repairsHandle=${trivialName}" class="csstest" style="color:#6699cc;">
        	处理
        	</a>
		</c:if>
<!-- 第二次处理 -->
		<c:if test="${repairs.repairsStatus == '维护中'}">
       		<a href="${ctx}/repairs/updateById/?repairsId=${repairs.repairsId}&category=${repairs.repairsCategory}&repairsStatus=已维护&repairsHandle=${trivialName}" class="csstest" style="color:#6699cc;">
        	处理
        	</a>
		</c:if>
		...
		<c:if test="${repairs.repairsStatus == '待绿化'}">
       		<a href="${ctx}/repairs/updateById/?repairsId=${repairs.repairsId}&category=${repairs.repairsCategory}&repairsStatus=绿化中&repairsHandle=${trivialName}" class="csstest" style="color:#6699cc;">
        	处理
        	</a>
		</c:if>
		
		<c:if test="${repairs.repairsStatus == '绿化中'}">
       		<a href="${ctx}/repairs/updateById/?repairsId=${repairs.repairsId}&category=${repairs.repairsCategory}&repairsStatus=已绿化&repairsHandle=${trivialName}" class="csstest" style="color:#6699cc;">
        	处理
        	</a>
		</c:if>
		
		
        <c:if test="${repairs.repairsStatus == '已维护' or repairs.repairsStatus == '已保洁' or repairs.repairsStatus == '已绿化'}">
			已处理
		</c:if>
...
<div style="width:100%; text-align:center;margin-top:10px;"> 当前第${currentPage}/${totalPage}页&nbsp;
  ...
  if ('${privilege}'!='1' || '${userCategory}'!='工作人员' || '${userDepartment}'!='环境部') {
  	...
  	function jump(){
  		...
  		$.ajax({
			type: "get",
			url: href="${ctx}/repairs/selectLimit?currentPage="+jumpPage+"&repairsCategory=${repairsCategory}",
			dataType: 'html',
  	...
  	function first(){
  		$.ajax({
			type: "get",
			url: href="${ctx}/repairs/selectLimit?currentPage=1"+"&repairsCategory=${repairsCategory}",
			dataType: 'html',
			success: function(result) {
				$("#info").html(result);
			},
  	...
  	function previ(){
  		$.ajax({
			type: "get",
			url: href="${ctx}/repairs/selectLimit?currentPage=${currentPage-1}"+"&repairsCategory=${repairsCategory}",
			dataType: 'html',
			success: function(result) {
				$("#info").html(result);
			},
  	...
  	function last(){
  		$.ajax({
			type: "get",
			url: href="${ctx}/repairs/selectLimit?currentPage=${totalPage}"+"&repairsCategory=${repairsCategory}",
			dataType: 'html',
			success: function(result) {
				$("#info").html(result);
			},
