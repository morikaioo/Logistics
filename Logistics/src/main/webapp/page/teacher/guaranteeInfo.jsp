...
<table class="csstest" cellspacing="0" cellpadding="0" id="ContentPlaceHolder1_dg1" style="border-width:0px;border-style:None;width:100%;border-collapse:collapse;">
	<tbody><tr align="left" style="color:#999999;background-color:White;height:20px;">
	 ...
	 <c:forEach items="${repairss}" var="repairs" varStatus="status">
		...
		<td style="width:45%;">
        <a href="${ctx}/repairs/selectById/${repairs.repairsId}?category=j" class="csstest">
...
<div style="width:100%; text-align:center;margin-top:10px;"> 当前第${currentPage}/${totalPage}页&nbsp;
  ...
  <script type="text/javascript">
  	...
  	function jump(){
  		...
  		$.ajax({
			type: "get",
			url: href="${ctx}/repairs/selectLimit?currentPage="+jumpPage+"&repairsCategory=${repairsCategory}"+"&judge=j",
			dataType: 'html',
  	...
  	function first(){
  		$.ajax({
			type: "get",
			url: href="${ctx}/repairs/selectLimit?currentPage=1"+"&repairsCategory=${repairsCategory}"+"&judge=j",
			dataType: 'html',
			success: function(result) {
				$("#info").html(result);
			},
  	...
  	function previ(){
  		$.ajax({
			type: "get",
			url: href="${ctx}/repairs/selectLimit?currentPage=${currentPage-1}"+"&repairsCategory=${repairsCategory}"+"&judge=j",
			dataType: 'html',
			success: function(result) {
				$("#info").html(result);
			},
  	...
  	function last(){
  		$.ajax({
			type: "get",
			url: href="${ctx}/repairs/selectLimit?currentPage=${totalPage}"+"&repairsCategory=${repairsCategory}"+"&judge=j",
			dataType: 'html',
			success: function(result) {
				$("#info").html(result);
			},
